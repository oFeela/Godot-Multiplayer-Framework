...
## spawn player characters at designated [Marker2D] locations, handle death/respawn timers,
## and manage scene navigation upon server disconnection.
class_name MultiplayerWorld2D
extends Node2D
...
## [param target_position]: The target spatial position ([Vector2]).
func force_reposition(player: Player, target_position: Vector2) -> void:
	if not RunService.is_server():
		return
...
func _rpc_force_reposition(target_position: Vector2) -> void:
...
## Recursively scans the scene hierarchy to locate all active [Marker2D] nodes.
func _find_spawn_points(curr_node: Node) -> Array[Marker2D]:
	var results: Array[Marker2D] = []
	
	if curr_node is Marker2D:
